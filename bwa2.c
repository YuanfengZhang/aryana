...
void compute_penalty(aryana_args *args, penalty_t *p) {
    if (args->mismatch_limit >= 0 && p->mismatch_num > args->mismatch_limit) p->penalty = maxPenalty;
        ...
        p->penalty = args->mismatch_penalty * p->mismatch_num + args->gap_open_penalty * p->gap_open_num +
                     args->gap_ext_penalty * p->gap_ext_num;
}

void OutputBufferError() {
    fprintf(stderr, "Error: Output buffer size is insufficient for preserving the order of reads.\n");
    fprintf(stderr,
            "Either set a larger size for output buffer using -B/--buffer argument, or don't use -O/--order argument.\n");
    exit(1);
}
...
void OutputBufferWarning() {
    ...
    fprintf(stderr,
            "Warning: Output buffer size is low for preserving the order of reads. This might reduce the speed.\n");
...
void PrintRefSeq(uint64_t *reference, unsigned long long start, unsigned long long end, unsigned long long seq_len,
                 int endl) {
...
void GetRefSeq(uint64_t *reference, unsigned long long start, unsigned long long end, unsigned long long seq_len,
               char *seq) {
    unsigned long long k;
    for (k = start; k <= end; k++)
...
void compute_cigar_penalties(global_vars *g, int candidates_num, int candidates_size, int * candidates, char * cigar[], bwa_seq_t * seq, hash_element * table, int **d, unsigned char **arr, char *tmp_cigar, penalty_t * penalty) {
    int i;
    for (i = candidates_num; i < candidates_size; i++) // The array is started to fill from the last.
        if (candidates[i] != -1) {
            create_cigar(g->args, &table[candidates[i]], cigar[i], seq->len, seq->seq, seq->qual, g->bwt->seq_len, d,
                         arr, tmp_cigar, penalty + i, g->reference, g->args->ignore);
            ...
            if (g->args->debug >= 1) {
                fprintf(stderr, "index: %llu, cigar: %s, mimatch %d, gap open %d, gap ext %d, penalty %d\n",
                        (unsigned long long) table[candidates[i]].index, cigar[i], penalty[i].mismatch_num,
                        penalty[i].gap_open_num, penalty[i].gap_ext_num, penalty[i].penalty);
                PrintSeq(seq->seq, seq->len, 1);
                PrintRefSeq(g->reference, table[candidates[i]].index, table[candidates[i]].index + seq->len - 1,
                            g->bwt->seq_len, 1);
            }
        }
}
...
int valid_pair(global_vars *g, hash_element *hit1, hash_element *hit2, bwtint_t len1, bwtint_t len2) {
    ...
    if (fragLen < g->args->min_dis || fragLen > g->args->max_dis)
        return 0;    // The length of fragment does not match the input arguments
    ...
    for (i = 0; cigar[i]; i++) {
        ...
        for (; num > 0; read_index++, refrence_index++, num--) {
            ...
            if (seq[read_index] == getNuc(refrence_index, g->reference, g->bwt->seq_len)) {
                ...
                p_ans *= ((1 - p_snp) * (p_mismatch / 3) + (p_snp / 3) * (1 - p_mismatch) +
                          (2 * p_snp / 3) * (p_mismatch / 3)) * p_m;
            }
...
void
compute_mapq(global_vars *g, int *can, int *can2, int num, penalty_t *p, penalty_t *p2, bwtint_t len, bwtint_t len2,
             ubyte_t *seq, ubyte_t *seq2, ubyte_t *qual, ubyte_t *qual2, hash_element *t, hash_element *t2, char *c[],
             char *c2[], int paired) {
...
void find_best_candidates(global_vars *g, int candidates_num, int candidates_num2, int candidates_size, int *candidates,
                          int *candidates2, penalty_t *penalty, penalty_t *penalty2, int *best_candidates,
                          int *best_candidates2, int *best_num, hash_element *table, hash_element *table2, bwtint_t len,
                          bwtint_t len2, ubyte_t *seq, ubyte_t *seq2, ubyte_t *qual, ubyte_t *qual2, char *cigar[],
                          char *cigar2[], int paired) {
    *best_num = 0;
        ...
        if (candidates[i] != -1) {
            if (!paired) {    // Single-end reads
                ...
                for (j = candidates_num2; j < candidates_size; j++) {
                    if (candidates2[j] != -1 &&
                        valid_pair(g, table + candidates[i], table2 + candidates2[j], len, len2)) {
                        if (min_penalty >
                            penalty[i].penalty + penalty2[j].penalty) { // improving paired-alignment penalty
                            min_penalty = penalty[i].penalty + penalty2[j].penalty;
                            *best_num = 0;
                        }
                        if (*best_num < candidates_size && min_penalty < maxPenalty && min_penalty ==
                                                                                       penalty[i].penalty +
                                                                                       penalty2[j].penalty) { // A new paired alignment with the same penalty
    ...
    if (*best_num > 0)
        compute_mapq(g, best_candidates, best_candidates2, *best_num, penalty, penalty2, len, len2, seq, seq2, qual,
                     qual2, table, table2, cigar, cigar2, paired);
}

long long
report_alignment_results(global_vars *g, char *buffer, char *cigar[], char *cigar2[], bwa_seq_t *seq, bwa_seq_t *seq2,
                         hash_element *table, hash_element *table2, int *can, int *can2, int canNum, int canNum2,
                         penalty_t *penalty, penalty_t *penalty2) {
    long long result = 0;
    int i, p, flag;
    if (!g->args->paired) {
        if (!canNum)
            return sam_generator(buffer + result, seq->name, sf_unmapped, 0, 0, 0, 0, 0, seq->seq, seq->qual, seq->len,
                                 0, g->bwt, g->offset, g->offInd);
        p = rand() % canNum; // primary alignment
        result += sam_generator(buffer + result, seq->name, 0, penalty[p].mapq, table[can[p]].index, cigar[p], 0, 0,
                                seq->seq, seq->qual, seq->len, 0, g->bwt, g->offset, g->offInd);
        if (g->args->report_multi)
            for (i = 0; i < canNum; i++)
                if (i != p)
                    result += sam_generator(buffer + result, seq->name, sf_not_primary, penalty[i].mapq,
                                            table[can[i]].index, cigar[i], 0, 0, seq->seq, seq->qual, seq->len, 0,
    ...
    if (!canNum) {
        ...
        result += sam_generator(buffer + result, seq->name, flag | sf_first_mate, 0, 0, 0, 0, 0, seq->seq, seq->qual,
                                seq->len, 0, g->bwt, g->offset, g->offInd);
        result += sam_generator(buffer + result, seq2->name, flag | sf_second_mate, 0, 0, 0, 0, 0, seq2->seq,
                                seq2->qual, seq2->len, 0, g->bwt, g->offset, g->offInd);
        return result;
    }
    flag |= sf_pair_mapped;
    p = rand() % canNum;
    result += sam_generator(buffer + result, seq->name, flag | sf_first_mate, penalty[p].mapq, table[can[p]].index,
                            cigar[p], table2[can2[p]].index, cigar2[p], seq->seq, seq->qual, seq->len, seq2->len,
                            g->bwt, g->offset, g->offInd);
    result += sam_generator(buffer + result, seq2->name, flag | sf_second_mate, penalty2[p].mapq, table2[can2[p]].index,
                            cigar2[p], table[can[p]].index, cigar[p], seq2->seq, seq2->qual, seq2->len, seq->len,
                            g->bwt, g->offset, g->offInd);
    if (g->args->report_multi) {
        flag |= sf_not_primary;
        for (i = 0; i < canNum; i++)
            if (i != p) {
                result += sam_generator(buffer + result, seq->name, flag | sf_first_mate, penalty[i].mapq,
                                        table[can[i]].index, cigar[i], table2[can2[i]].index, cigar2[i], seq->seq,
                                        seq->qual, seq->len, seq2->len, g->bwt, g->offset, g->offInd);
                result += sam_generator(buffer + result, seq2->name, flag | sf_second_mate, penalty2[i].mapq,
                                        table2[can2[i]].index, cigar2[i], table[can[i]].index, cigar[i], seq2->seq,
                                        seq2->qual, seq2->len, seq->len, g->bwt, g->offset, g->offInd);
            }
    }
    return result;
}
...
unsigned char **arr_a[MAX_THREADS_COUNT];
...
void multiAligner(global_vars *g) {
    ...
    if (d_a[my_mem] == 0) {
        ...
        unsigned char* te_arr = malloc(MAX_READ_LEN * 300 * (sizeof(char)));
    ...
    int **d = d_a[my_mem];
    unsigned char **arr = arr_a[my_mem];
    ...
    while (true) {
        ...
        if ((seqs = bwa_read_seq(g->ks, seq_num_per_file_read, &n_seqs, g->opt->mode, g->opt->trim_qual)) == 0 ||
            (g->args->paired &&
             (seqs2 = bwa_read_seq(g->ks2, seq_num_per_file_read, &n_seqs2, g->opt->mode, g->opt->trim_qual)) == 0))
            read = 0;
        ...
        for (j = 0; j < n_seqs; j++) {
            ...
            struct report *rpt = align_read(g, buffer, cigar, cigar2, &seqs[j], (g->args->paired) ? &seqs2[j] : 0,
                                            table, table2, total_seqs + j + 1, d, arr, tmp_cigar);
            report_alignment_results(rpt->g, rpt->buffer, rpt->cigar, rpt->cigar2, rpt->seq, rpt->seq2, rpt->table,
                                     rpt->table2, rpt->can, rpt->can2, rpt->canNum, rpt->canNum2, rpt->penalty,
                                     rpt->penalty2);
